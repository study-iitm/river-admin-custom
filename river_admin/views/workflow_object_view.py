...
@get(r'^workflow-object/identify/(?P<workflow_pk>\w+)/(?P<object_id>[^/]+)/$')
...
@get(r'^workflow-object/current-state/(?P<workflow_pk>\w+)/(?P<object_id>[^/]+)/$')
...
@get(r'^workflow-object/current-iteration/(?P<workflow_pk>\w+)/(?P<object_id>[^/]+)/$')
...
@delete(r'^workflow-object/delete/(?P<workflow_pk>\w+)/(?P<object_id>[^/]+)/$')
def get_identifier(request, workflow_pk, object_id):
    workflow = get_object_or_404(Workflow.objects.all(), pk=workflow_pk)
    model_class = workflow.content_type.model_class()
    workflow_object = get_object_or_404(model_class.objects.all(), pk=object_id)
...
@get(r'^workflow-object/state/list/(?P<workflow_id>\w+)/(?P<object_id>[^/]+)/$')
...
@get(r'^workflow-object/transition/list/(?P<workflow_id>\w+)/(?P<object_id>[^/]+)/$')
...
@get(r'^workflow-object/transition-approval/list/(?P<workflow_id>\w+)/(?P<object_id>[^/]+)/$')
